...
6. `parse_adapter_output()` извлекает дополнительные findings из stdout/stderr и generated report text для поддерживаемых adapter families, включая username/email/phone adapters и domain-recon adapters Subfinder/httpx/passive Amass/theHarvester.
...
5. При `--execute-adapters` совместимые adapters запускаются через `run_adapter_findings()`; stdout/stderr parser добавляет дополнительные adapter findings. Для domain seeds профиль `domain-recon` может добавить Subfinder/httpx/passive Amass/theHarvester, а их parsed findings входят в тот же graph.
...
`domain/url seed -> domain-recon profile -> subfinder/httpx/passive amass/theHarvester -> JSONL/plain stdout/generated JSON -> subdomain/email/url/domain metadata -> Entity[]/GraphEdge[]`
