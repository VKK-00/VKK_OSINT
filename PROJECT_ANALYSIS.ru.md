...
- Adapter manifest теперь включает executable target для `h8mail`, target-specific executable templates для `user-scanner` и region-aware template для `snooppr/snoop`; более сложные adapters могут потребовать richer per-mode config.
- Adapter parser покрывает общие URL/email/phone/key-value patterns, `user-scanner` JSON/verbose output и Snoop stdout/CSV output; сложные JSON/CSV/HTML exports остальных upstream ещё не разобраны.
