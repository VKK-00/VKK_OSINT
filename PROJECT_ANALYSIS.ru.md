...
- generated report ingestion: внешние adapters могут писать JSON/CSV в временную output-папку, после чего runner читает эти файлы и передаёт их в parser;
...
5. Если adapter объявляет generated report files, runner создаёт временную output-папку, добавляет upstream-аргумент output folder и читает сгенерированные файлы.
...
- Generated report files читаются из временной директории и удаляются после parsing; постоянное хранение остаётся задачей case store/report output.
...
- Adapter manifest теперь включает executable target для `h8mail`, generated JSON-report template для `soxoj/maigret`, target-specific executable templates для `user-scanner` и region-aware template для `snooppr/snoop`; более сложные adapters могут потребовать richer per-mode config.
- Adapter parser покрывает общие URL/email/phone/key-value patterns, Maigret JSON/CSV reports, `user-scanner` JSON/verbose output и Snoop stdout/CSV output; сложные JSON/CSV/HTML exports остальных upstream ещё не разобраны.
