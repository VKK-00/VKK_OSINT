...
- Adapter manifest теперь включает executable target для `h8mail` и target-specific executable templates для `user-scanner`; более сложные adapters могут потребовать richer per-mode config.
- Adapter parser покрывает общие URL/email/phone/key-value patterns и `user-scanner` JSON/verbose output; сложные JSON/CSV/HTML exports остальных upstream ещё не разобраны.
