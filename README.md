...
- выполнять baseline domain recon: DNS resolution, HTTP/HTTPS metadata, bounded same-site crawler, public email/phone/social link extraction, security header presence, certificate transparency subdomain discovery и RDAP registration lookup;
...
- получать базовые web metadata, public email extraction и bounded same-site crawl по URL;
...
Native domain module является baseline-слоем для web-check/Photon/theHarvester/SpiderFoot/Amass/Subfinder-подобного web/domain recon. В `--live` режиме он делает DNS resolution, HTTP/HTTPS metadata, public email extraction из уже загруженных landing pages, bounded same-site crawler (`--crawl-pages`, `--crawl-depth`), security header presence, certificate transparency lookup через `crt.sh` и RDAP lookup через `rdap.org`. CT names попадают как `subdomain` entities и graph edges `domain -> subdomain`; RDAP registrar/nameservers попадают как `registrar`/`nameserver` entities и graph edges `domain -> registrar|nameserver`; найденные emails/phones/social URLs попадают как `email`/`phone`/`url` entities и graph edges `domain|url -> email|phone|url`. Он не делает brute force, port scanning, raw WHOIS text lookup, JavaScript rendering, form submission или Amass/Subfinder-подобное широкое перечисление.

Native URL scan использует тот же bounded crawler: стартовая страница извлекает title/status/content-type, затем до заданного лимита обходит same-site HTTP(S)-ссылки и собирает public email, E.164-like phone values, external links и social links. По умолчанию лимиты небольшие: 5 страниц и глубина 1.
...
Отчёт содержит `Entity Summary` и `Graph Edges`: нормализованные сущности и связи между ними, например `email -> domain`, `email -> related_email`, `domain -> email`, `domain -> phone`, `domain -> discovered URL`, `domain -> social URL`, `domain -> subdomain`, `domain -> registrar`, `domain -> nameserver`, `url -> domain`, `telegram -> url`, `phone -> country`.
