...
Native email/phone modules являются baseline-слоем для Mosint/h8mail/pwnedOrNot/user-scanner/PhoneInfoga-подобных adapters. Phone module делает E.164-like normalization, country-prefix signal и статические numbering-plan hints для Украины и зоны `+7`: `line_type`, `carrier` для украинских mobile allocation prefixes, `location` для известных fixed-line NDC и `country_code` для RU/KZ split. Carrier — это allocation hint, а не live carrier lookup: переносимость номера может изменить фактического оператора. Email module делает local-part profile hints без сети: role/shared mailboxes (`support@`, `admin@`) помечаются как `skipped`, handle-like local parts получают `username`, а `first.last+tag@` дополнительно получает `name`, `base_local_part` и `plus_tag`. В `--live` режиме email module делает domain resolution, MX/NS/TXT lookup через системный `nslookup`, hosted provider attribution по MX/NS/TXT clues для Google Workspace, Microsoft 365, Yandex Mail, Mail.ru, Proton Mail, Zoho, Fastmail, Amazon SES, iCloud и Cloudflare Email Routing, certificate transparency domain correlation через `crt.sh`, SPF classification по доменному TXT, DMARC через `_dmarc.<domain>`, MTA-STS через `_mta-sts.<domain>`, TLS-RPT через `_smtp._tls.<domain>`, BIMI через `default._bimi.<domain>` и распознаёт публичные TXT service signals вроде Google/Microsoft/Yandex/Mail.ru verification markers. Он не делает breach lookup, account-enumeration и не запускает password recovery flows.
