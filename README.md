...
- выполнять baseline email checks: синтаксис, live domain resolution, MX и TXT lookup;
...
Native email/phone modules являются baseline-слоем для Mosint/h8mail/pwnedOrNot/PhoneInfoga-подобных adapters. Email module в `--live` режиме делает domain resolution и MX/TXT lookup через системный `nslookup`, если он доступен в `PATH`. Он не делает breach lookup, account-enumeration и не запускает password recovery flows.
