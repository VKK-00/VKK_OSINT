...
- выполнять baseline domain recon: DNS resolution, HTTP/HTTPS metadata, security header presence, certificate transparency subdomain discovery и RDAP registration lookup;
...
Native domain module является baseline-слоем для web-check/theHarvester/SpiderFoot/Amass/Subfinder-подобного web/domain recon. В `--live` режиме он делает DNS resolution, HTTP/HTTPS metadata, security header presence, certificate transparency lookup через `crt.sh` и RDAP lookup через `rdap.org`. CT names попадают как `subdomain` entities и graph edges `domain -> subdomain`; RDAP registrar/nameservers попадают как `registrar`/`nameserver` entities и graph edges `domain -> registrar|nameserver`. Он не делает brute force, port scanning, raw WHOIS text lookup или Amass/Subfinder-подобное широкое перечисление.
...
Отчёт содержит `Entity Summary` и `Graph Edges`: нормализованные сущности и связи между ними, например `email -> domain`, `email -> related_email`, `domain -> subdomain`, `domain -> registrar`, `domain -> nameserver`, `url -> domain`, `telegram -> url`, `phone -> country`.
