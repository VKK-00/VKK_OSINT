...
- выполнять baseline domain recon: DNS resolution, HTTP/HTTPS metadata, security header presence и certificate transparency subdomain discovery;
...
Native domain module является baseline-слоем для web-check/theHarvester/SpiderFoot/Amass/Subfinder-подобного web/domain recon. В `--live` режиме он делает DNS resolution, HTTP/HTTPS metadata, security header presence и certificate transparency lookup через `crt.sh`, после чего найденные имена попадают как `subdomain` entities и graph edges `domain -> subdomain`. Он не делает brute force, port scanning, WHOIS/RDAP или Amass/Subfinder-подобное широкое перечисление.
...
Отчёт содержит `Entity Summary` и `Graph Edges`: нормализованные сущности и связи между ними, например `email -> domain`, `email -> related_email`, `domain -> subdomain`, `url -> domain`, `telegram -> url`, `phone -> country`.
