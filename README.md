...
- запускать Subfinder, httpx, пассивный Amass, theHarvester и BBOT domain-recon adapters и нормализовать subdomains/emails/URLs/IPs/ports/technologies/HTTP probe results в общий graph;
...
python -m osint_toolkit investigate --domain example.com --include-adapters --adapter-profile domain-recon --adapter-limit 5
...
Текущие профили включают `username-full`, `username-ru-ua`, `email-safe`, `phone-safe`, `url-archive` и `domain-recon`. `username-full` включает Sherlock через `sherlock <username> --no-color --print-all --csv --txt --folderoutput <tempdir>`, Maigret через `maigret <username> --json ndjson`, Nexfil через изолированный temporary workdir с autosaved TXT reports, `instaloader profile <profile>` для Instagram edge cases, а также `user-scanner`; `username-ru-ua` начинается со Snoop и Maigret. При `--region ru|ua` Snoop получает `--include RU|UA`, а Maigret получает `--tags ru|ua`. `email-safe` включает Mosint через `mosint --silent <email> --output <temp.json>`, h8mail через `h8mail -t <email> --hide -j <temp.json>` и `user-scanner` через target-specific JSON-команды `user-scanner -u <username> -f json` и `user-scanner -e <email> -f json`. `phone-safe` включает PhoneInfoga через `phoneinfoga scan -n <number>`; GPL-код PhoneInfoga не копируется внутрь проекта, но stdout/API-like JSON вывод приводится к общим `Finding`/`Entity`/graph signals. `domain-recon` включает Subfinder, httpx, пассивный Amass, theHarvester и BBOT в passive `subdomain-enum` режиме. Restricted email-to-account/email-to-phone adapters в safe-профили не входят.
