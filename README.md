...
- выполнять baseline domain recon: DNS resolution, HTTP/HTTPS metadata, public page email extraction, security header presence, certificate transparency subdomain discovery и RDAP registration lookup;
...
- получать базовые web metadata и public email extraction по URL;
...
Native domain module является baseline-слоем для web-check/theHarvester/SpiderFoot/Amass/Subfinder-подобного web/domain recon. В `--live` режиме он делает DNS resolution, HTTP/HTTPS metadata, public email extraction из уже загруженных landing pages, security header presence, certificate transparency lookup через `crt.sh` и RDAP lookup через `rdap.org`. CT names попадают как `subdomain` entities и graph edges `domain -> subdomain`; RDAP registrar/nameservers попадают как `registrar`/`nameserver` entities и graph edges `domain -> registrar|nameserver`; найденные emails попадают как `email` entities и graph edges `domain -> email`. Он не делает brute force, port scanning, raw WHOIS text lookup, recursive crawler или Amass/Subfinder-подобное широкое перечисление.
...
Отчёт содержит `Entity Summary` и `Graph Edges`: нормализованные сущности и связи между ними, например `email -> domain`, `email -> related_email`, `domain -> email`, `domain -> subdomain`, `domain -> registrar`, `domain -> nameserver`, `url -> domain`, `telegram -> url`, `phone -> country`.
